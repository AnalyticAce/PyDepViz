...
@app.command(
    name="visualize",
    help="Visualize project dependencies in various formats."
)
def visualize(
    input_file: Path = typer.Argument(..., exists=True, file_okay=True, dir_okay=False, resolve_path=True, help="Path to the dependency file to visualize"),
    format: OutputFormat = typer.Option(OutputFormat.HTML, "-f", "--format", case_sensitive=False, help="Output format for the visualization")
) -> None:
    typer.echo(f"Visualizing dependencies from {input_file} in {format.value} format.")

@app.command(
    name="serve",
    help="Start a web server for interactive dependency exploration."
)
def serve(
    port: int = typer.Option(8000, "-p", "--port", min=1024, max=65535, help="The port to run the server on", show_default=True),
    host: str = typer.Option("127.0.0.1", "-h", "--host", help="The host interface to bind to"),
...
@app.command(
    name="export",
    help="Export dependencies to an image format."
)
def export(
...
@app.command(
    name="analyze",
    help="Analyze dependencies and generate a detailed graph."
)
def analyze(
...
@app.callback(
    ...
    help="Dependency Analyzer - A CLI tool for analyzing and visualizing project dependencies."
)
def main(
